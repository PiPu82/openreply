...
export default function SettingsPage() {
  ...
  async function disconnectInstagram(instagramAccountId: string) {
    if (!confirm("Instagram trennen? Kampagnen für dieses Konto versenden dann keine DMs mehr.")) {
      return;
    }
  ...
  async function inviteMember(event: React.FormEvent) {
    ...
    if (payload.success) {
      ...
      setMemberError(payload.error ?? "Mitglied konnte nicht eingeladen werden");
    }
    setBusy(null);
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <section className="panel rounded p-4 sm:p-6">
        <h2 className="text-base font-semibold mb-6">Instagram-Verbindung</h2>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-3 py-3 border-b border-border">
            ...
            >
              {accounts.length > 0 ? "Verbunden" : "Nicht verbunden"}
            </span>
          </div>

          <div className="flex items-center justify-between gap-3 py-3 border-b border-border">
            <div>
              <p className="text-sm font-medium text-foreground">Konten</p>
              <p className="text-xs text-muted mt-0.5">
            ...
            <span className="text-sm text-muted">
              {accounts.length > 0 ? `${accounts.length} connected` : "Keine"}
            </span>
          </div>
        ...
        <div className="mt-6 pt-4 border-t border-border flex gap-3">
          ...
          >
            {accounts.length > 0 ? "Weiteres Konto verbinden" : "Instagram verbinden"}
          </a>
        </div>
      </section>
      ...
      <section className="panel rounded p-4 sm:p-6">
            ...
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-foreground">
                  {member.user.name ?? member.user.email ?? "Unbekanntes Mitglied"}
                </p>
        ...
        {canManageMembers && (
          ...
          >
            ...
            >
              {busy === "invite" ? "Wird eingeladen…" : "Einladen"}
            </button>
            {memberError && (
      ...
      <section className="panel rounded p-4 sm:p-6">
        <h2 className="text-base font-semibold mb-6">Nutzung</h2>
